...
NEW: The broadcast loop now reads the last detected object from Redis and
sends it to the frontend via a new 'ai_update' WebSocket message.
"""
import asyncio
    ...
    async def broadcast_updates():
        while True:
            try:
                ...
                # --- Get the last detected object name from Redis ---
                last_detection = await app.state.redis_client.get(AI_LAST_DETECTION_KEY)
                ...
                # --- Send the new message type to the frontend ---
                await websocket_manager.broadcast_json({
                    "type": "ai_update", 
                    "data": {"last_detected": last_detection or "Searching..."}
                })

                await asyncio.sleep(0.5) # The loop already runs at 2Hz

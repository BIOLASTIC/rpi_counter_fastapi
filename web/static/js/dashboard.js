...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- DOM Element Cache ---
    // Cache all elements with an ID for faster access
    const ui = new Map(
        ...
        socket.onmessage = (event) => {
            try {
                ...
                if (message.type === 'full_status') {
                    // Store the latest state and update the entire UI
    ...
    function updateSystemStatusUI(system) {
        ...
        // Update all toggle switches based on the backend's source of truth
    ...
    document.querySelectorAll('.manual-control-toggle input[type="checkbox"]').forEach(toggle => {
        toggle.addEventListener('click', function(event) {
            // Prevent the UI from changing instantly. The change will happen when the WebSocket confirms it.
